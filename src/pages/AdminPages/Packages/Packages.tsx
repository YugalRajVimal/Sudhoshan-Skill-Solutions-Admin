...
import { FiEdit, FiX, FiCheck, FiPlus, FiMinus } from "react-icons/fi";
...
// New type for editing package which stores features as an array
type EditPkgType = {
  name: string;
  price: string;
  tasksPerDay: string;
  taskRate: string;
  bv: string;
  features: string[]; // each feature as a separate input
};
...
export default function PackagesPage() {
  ...
  const [editPkg, setEditPkg] = useState<EditPkgType>({
    name: "",
    price: "",
    tasksPerDay: "",
    taskRate: "",
    bv: "",
    features: [], // start with 0 features
  });
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const features = editPkg.features.map(f => f.trim()).filter(Boolean);
  ...
  // Add new empty feature field during editing
  ...
  const handleFeatureChange = (index: number, value: string) => {
    setEditPkg((p) => {
      const nxt = [...p.features];
      nxt[index] = value;
      return { ...p, features: nxt };
    });
  };
            ...
            <tbody>
              {packages.length === 0 ? (
                ...
                packages.flatMap((pkg) => {
                  if (editId === pkg._id) {
                    // Render two rows: the main editing row and the full-width features row underneath
                    return [
                      ...
                      <tr key={`${pkg._id}-features`} className="bg-yellow-50">
                        <td colSpan={7} className="px-4 pb-4 pt-0">
                          <div className="font-medium mb-2 mt-0.5 text-slate-700 text-xs uppercase tracking-wide">Features</div>
                          <div className="max-w-3xl space-y-2">
                            {editPkg.features.length === 0 && (
                              <div className="text-slate-400 text-xs">(No features)</div>
                            )}
                            {editPkg.features.map((feature, fi) => (
                              <div key={fi} className="flex items-center gap-1">
                                <input
                                  className="w-full border rounded px-2 py-1"
                                  value={feature}
                                  onChange={e =>
                                    handleFeatureChange(fi, e.target.value)
                                  }
                                  onKeyDown={e => {
                                    if (e.key === "Escape") cancelEdit();
                                  }}
                                ...
                                <button
                                  ...
                                  className="ml-1 text-xs border p-1 rounded hover:bg-red-50"
                                  title="Remove feature"
                                  disabled={editPkg.features.length <= 1 || submitting}
                                >
                                  <FiMinus />
                                </button>
                              </div>
                            ...
                            <button
                              ...
                              className="mt-1 text-xs border border-blue-300 text-blue-700 px-2 py-0.5 rounded flex items-center gap-1"
                              disabled={submitting}
                              title="Add feature"
                            >
                              <FiPlus /> Add Feature
